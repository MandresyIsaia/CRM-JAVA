    ...
    public List<Customer> importCustomerCsv(MultipartFile file, char separator, User user) throws Exception {
        ...
        try (BufferedReader reader = new BufferedReader(new InputStreamReader(file.getInputStream(), StandardCharsets.UTF_8));
            ...
            if (!csvParser.getHeaderMap().containsKey("customer_email") ||
                ...
                throw new Exception("Le fichier CSV ne contient pas les en-têtes requis.");
            }
            ...
            for (CSVRecord record : csvParser) {
                try {
                    ...
                    if (email.isEmpty() || name.isEmpty()) {
                        throw new Exception("Ligne ignorée : Email ou Nom vide. Ligne " + record.getRecordNumber());
                    }
                    ...
                    throw new Exception("Erreur lors de l'analyse de la ligne " + record.getRecordNumber() + ": " + e.getMessage());
                }
            }
            ...
            for (Customer customer : customerList) {
                try {
                    customerList1.add(customerService.save(customer));
                } catch (Exception e) {
